	...
	def pdf_to_png(pdf_name, res,file_no):
		...
		if inputpdf.numPages:
			for i in xrange(inputpdf.numPages):
				...
				a1 = "{\"sourceImage\": {}, \"templateRequests\": [{}],\"textRequests\": [],\"maskingAreas\": []}"
				a2 = "{\"pageID\" : \"123\", \"templateOutput\" : [{}]}"
				...
				with Image(file=temp_image,  resolution=res) as img:
					with Image(width=img.width, height=img.height, background=Color("white")) as bg:
						...
						cursor.execute("""INSERT INTO drawings(page_number,image,pdf,request_json,result_json) VALUES (%s,%s, %s, %s, %s)""", (pgn,png_bin,rpdf_page,a1,a2))
						db.commit()
				#rpdf_page.close()
		db.close()
